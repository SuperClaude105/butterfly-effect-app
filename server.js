...
// ─── Cover prompt builder — genre-differentiated styles ──────────────────
const GENRE_STYLES = {
  // ── Romance family ─────────────────────────────────────────────────────────
  'Dark Romance':
...
function buildCoverPrompt({ genre, bookTitle, premise, themes, protagonistName, loveInterestName }) {
  ...
  const parts     = [
    `Book cover for "${bookTitle}", a ${g} novel.`,
    premise           ? ` Premise: ${premise.slice(0, 120)}.` : '',
    protagonistName   ? ` Protagonist: ${protagonistName}.`   : '',
    loveInterestName  ? ` Love interest: ${loveInterestName}.` : '',
    ` Themes: ${themeStr}.`,
    ` STYLE: ${style}`,
...
app.post('/api/generate-cover', async (req, res) => {
  ...
  const prompt = buildCoverPrompt({ genre, bookTitle, premise, themes, protagonistName, loveInterestName });
